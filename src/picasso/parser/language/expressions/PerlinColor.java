...
public class PerlinColor extends ExpressionTreeNode {
	...
	public RGBColor evaluate(double x, double y) {
		double xval = expr1.evaluate(x, y).getRed(); 
		double yval = expr2.evaluate( x, y).getBlue();
		Random rand = new Random();
		
		double noiseValueR = perlinNoise(xval * rand.nextInt(255), yval * rand.nextInt(255)); // Offset by randInt
        double noiseValueG = perlinNoise(xval * rand.nextInt(255), yval * rand.nextInt(255)); // Offset 
        double noiseValueB = perlinNoise(xval * rand.nextInt(255), yval * rand.nextInt(255)); // Offset 
		
		
        return new RGBColor(noiseValueR, noiseValueG, noiseValueB);
