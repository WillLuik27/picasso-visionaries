...
public class ExpressionTreeGeneratorTests {
	...
	public void wrapFunctionTests() {
		ExpressionTreeNode e= parser.makeExpression("wrap( x )" ); //ensure that expression is interpreted properly
		assertEquals(new Wrap(new X()), e);
		
		//now test with addition content within the parenthesis. Use binary function "+" from floor test
